...
## EV  fast  charging  stations  and  energy  storage  technolo-gies:  A real  implementation  in  the  smart micro  grid  paradigm
### Citation:
...
### Review:
### Notes:
- A FEMS means a Fuzzy Logic Energy Managements Strategy
- FLC used to maintain ultracapacitor charge state
- Minimizes battery current variations
- FLC tries to deteremine the power split between two energy storage sources
- Simulated driving cucles 
- Provides formula for LIon power cell and ultracapacitor 
- tries to manage so that it "feels" like a conventional vehicle
- tries to manage battery lifetime, effeciency and SOH, by regulating current dynamics
- Proporses an adaptive method to fuzzy logic management strategy
- propose that more driving cucles are used in simuatlions for future work
...
### Review:
### Notes:
- FLC is the simplest intelligent controller for inverter control
- FLC is better load variation,response time, settling time, robustness
- Inverters without transformers are favorable
- Multi-Level inverters are expensive and computationaly complex
- 3phase pv inverter using FLC
- Better than PI controller in reducing overshoots
- shows hwo to setup switching on a DC link to simulate 3 phase AC signals directly from PV system
- in off-grid mode the controller supplies volgtage controled power tot he ac loads
- Uses parks transformation for the voltage sensing
- FPLC minimizes error of between ref currents
- FLC uses trapezoidal membershi functions
- Created a physical prototype of the VSI using DSP board
- effecitvyle provided energy to the grid and sustained off-grid loads
...
### Review:
### Notes:
- reviews a city in india's real-time distribution system
- utilizes FLC to coordinate the demand of EV batteries
- shows a simulaiton of active and reactive power support
- FLC was used to fuzzfy the control logie to change power angle, duty ratio of converter
- no future work suggested
- good model of the system with electrical details 
- formulation for active and reactive power at a distirbution node


## (TBD) Energy managementfor  joint  operation  of  chp  and  pv  prosumers  inside  a  grid-connectedmicrogrid: A game theoretic approach
### Citation:
...
## An Integrated Fuzzy Logic Energy Management fora Dual-Source Electric Vehicle
### Citation:
...
### Review:
### Notes:
- good breakdown of how FLC works 
- focused on car power train
- has insights on pros/cons of FLC vs ANN , Dynamic programming , etc. 
- Shows a working simulation of FLC in practice highlighting optimization
- Future work is to implement a physical system from findings
...
## System Planning of Grid-Connected Electric Vehicle Charging Stations and Key Technologies
### Citation:
...
## Internet of Things-aided Smart Grid
### Citations:
